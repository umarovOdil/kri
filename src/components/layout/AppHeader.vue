<template>
  <header class="w-full bg-white dark:bg-[#1e212b] shadow-md z-50">
      ...
      <div class="max-w-[1440px] mx-auto py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="hidden sm:flex flex-col">
            <span
              class="text-[#273469] dark:text-white text-lg sm:text-lg font-extrabold leading-none tracking-tight uppercase"
              >Kompetensiyani Rivojlantirish</span
            >
            <span
              class="text-[12px] uppercase font-bold text-[#273469] leading-snug dark:text-white"
              >Instituti</span
            >
          </div>
        </div>
        ...
        <div class="flex items-center gap-6">
          <div class="hidden md:flex items-center gap-6 text-sm font-bold text-gray-500">
            ...
            <button
              @click="toggleTheme"
              class="hover:text-primary flex items-center gap-1 border border-gray-200 px-3 py-1 rounded-sm mr-2 transition-colors"
              :class="
                isDark ? 'bg-gray-800 text-yellow-400 border-gray-700' : 'bg-white text-gray-600'
              "
            >
            ...
            <div class="relative" ref="accessibilityMenu">
              <button
                ...
                class="hover:text-primary flex items-center gap-1 border border-gray-200 px-3 py-1 rounded-sm"
              >
              ...
              <div
                ...
                class="absolute top-full right-0 mt-2 w-72 bg-white dark:bg-[#1e212b] shadow-2xl rounded-xl p-6 z-[100] border border-gray-100 dark:border-gray-700"
              >
                <h3 class="text-lg font-bold text-[#121316] dark:text-white mb-4">
                  {{ t('nav.appearance') }}
                </h3>
                ...
                <div class="flex gap-3 mb-6">
                  <button
                    ...
                    class="w-14 h-14 rounded-lg bg-[#1e40af] text-white text-2xl font-bold flex items-center justify-center hover:opacity-90 transition-opacity ring-2 ring-offset-2 ring-transparent focus:ring-blue-500"
                  >
                    A
                  </button>
                  <button
                    @click="setTheme('gray')"
                    class="w-14 h-14 rounded-lg bg-[#757575] text-white text-2xl font-bold flex items-center justify-center hover:opacity-90 transition-opacity ring-2 ring-offset-2 ring-transparent focus:ring-gray-500"
                  >
                    A
                  </button>
                  <button
                    @click="setTheme('contrast')"
                    class="w-14 h-14 rounded-lg bg-[#333333] text-white text-2xl font-bold flex items-center justify-center hover:opacity-90 transition-opacity ring-2 ring-offset-2 ring-transparent focus:ring-black"
                  >
                    A
                  </button>
                </div>

                <h3 class="text-lg font-bold text-[#121316] dark:text-white mb-2">
                  {{ t('nav.font_size') }}
                </h3>
                <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
                  {{ fontSize }}% {{ t('nav.increase') }}
                </p>
            ...
            <div class="flex items-center border border-gray-200 rounded-sm overflow-hidden">
              <button
                ...
                :class="locale === 'uz' ? 'bg-primary text-white' : 'hover:bg-gray-100'"
              >
                O'ZB
              </button>
              <button
                @click="setLocale('ru')"
                class="px-3 py-1 text-xs transition-colors"
                :class="locale === 'ru' ? 'bg-primary text-white' : 'hover:bg-gray-100'"
              >
                РУС
              </button>
              <button
                @click="setLocale('en')"
                class="px-3 py-1 text-xs transition-colors"
                :class="locale === 'en' ? 'bg-primary text-white' : 'hover:bg-gray-100'"
              >
                ENG
              </button>
            </div>
          ...
          <button
            ...
            class="flex items-center gap-2 bg-primary hover:bg-[#1a254a] text-white px-5 py-2 transition-colors duration-200 shadow-lg shadow-blue-900/20 text-sm font-bold uppercase rounded-sm"
          >
          ...
          <button
            ...
            class="lg:hidden p-2 text-primary hover:bg-blue-50 rounded-lg"
          >
        ...
        <nav class="flex items-center text-[13px] font-bold uppercase tracking-wide">
          <div v-for="(item, index) in menuItems" :key="index" class="group relative">
            <router-link
              v-if="item.link"
              :to="item.link"
              class="block py-4 px-4 hover:bg-white/10 transition-colors flex items-center gap-1"
            >
              {{ item.title }}
            </router-link>
            <a
            ...
            >
              {{ item.title }}
              <span v-if="item.children" class="material-symbols-outlined text-[16px] opacity-70"
                >expand_more</span
              >
            </a>
            <router-link
              ...
              <ul class="py-2">
                ...
                >
                  <router-link
                    v-if="child.link"
                    :to="child.link"
                    class="block px-6 py-2.5 hover:bg-gray-50 hover:text-primary transition-colors border-b border-gray-50 last:border-0 flex justify-between items-center text-[12px]"
                  >
                    <span class="block pr-2">{{ child.title }}</span>
                  </router-link>
                  <a
                    v-else-if="child.subChildren || child.children"
                    href="#"
                    class="block px-6 py-2.5 hover:bg-gray-50 hover:text-primary transition-colors border-b border-gray-50 last:border-0 flex justify-between items-center text-[12px]"
                  >
                    <span class="block pr-2">{{ child.title }}</span>
                    <span class="material-symbols-outlined text-[14px] text-gray-400 shrink-0"
                      >chevron_right</span
                    >
                  </a>
                  <router-link
                  ...
                  >
                    <ul class="py-2 max-h-[500px] overflow-y-auto custom-scrollbar">
                      <li v-for="(sub, sIndex) in child.subChildren" :key="sIndex">
                        <router-link
                          to="/construction"
                          class="block px-6 py-2.5 hover:bg-gray-50 hover:text-primary text-[11px] leading-tight border-b border-gray-50 last:border-0"
                        >
                          {{ sub }}
                        </router-link>
                      </li>
                    ...
                    <ul class="py-2">
                      ...
                      >
                        ...
                        >
                          ...
                          <span
                            v-if="grandChild.subChildren"
                            class="material-symbols-outlined text-[14px] text-gray-400 shrink-0"
                            >chevron_right</span
                          >
                        </a>
    ...
    <div v-if="isMobileMenuOpen" class="fixed inset-0 z-[100] lg:hidden">
      ...
      <div
        class="absolute top-0 right-0 w-[85%] max-w-[320px] h-full bg-white dark:bg-[#1e212b] shadow-2xl overflow-y-auto"
      >
        <div
        ...
        >
          ...
          <button
            @click="toggleMobileMenu"
            class="p-2 hover:bg-gray-100 rounded-full dark:hover:bg-gray-800 transition-colors"
          >
        ...
        <div class="p-4">
          ...
          <nav class="flex flex-col gap-1">
            <div v-for="(item, index) in menuItems" :key="index">
              ...
              >
                <span
                  v-if="item.children"
                  class="font-bold text-sm text-[#121316] dark:text-white"
                  >{{ item.title }}</span
                >
                <router-link
                  v-else-if="item.link"
                  :to="item.link"
                  @click="toggleMobileMenu"
                  class="font-bold text-sm text-[#121316] dark:text-white flex-grow"
                >
                  {{ item.title }}
                </router-link>
                <router-link
              ...
              <!-- Mobile Submenu -->
              <div
                v-if="item.children && activeMobileSubmenu === index"
                class="pl-4 pr-2 pb-2 space-y-1"
              >
                <div
                  v-for="(child, cIndex) in item.children"
                  :key="cIndex"
                  class="py-2 border-l-2 border-gray-100 pl-3"
                >
                  <template v-if="child.link">
                    <router-link
                      :to="child.link"
                      @click="toggleMobileMenu"
                      class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1 hover:text-primary block"
                    >
                      {{ child.title }}
                    </router-link>
                  </template>
                    ...
                    <ul v-if="child.subChildren" class="space-y-2 mt-2">
                      <li v-for="(sub, sIndex) in child.subChildren" :key="sIndex">
                        <router-link
                          to="/construction"
                          @click="toggleMobileMenu"
                          class="text-xs text-gray-500 dark:text-gray-400 hover:text-primary block leading-tight"
                        >
                          {{ sub }}
                        </router-link>
                      </li>
          ...
          <div class="mt-8 pt-4 border-t border-gray-100 dark:border-gray-700">
            <button
              @click="showLoginModal = true"
              class="w-full bg-primary text-white py-3 rounded-lg font-bold flex items-center justify-center gap-2"
            >
...
const menuItems = computed<MenuItem[]>(() => [
  { title: t('menu.home'), link: '/' },
  {
    ...
    children: [
      ...
      { title: t('menu.institute.contact'), link: '/contact' },
    ],
  },
  {
  ...
  { title: t('menu.contact'), link: '/contact' },
])
</script>
